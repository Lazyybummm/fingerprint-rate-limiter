...
1. Middleware reads a unique `key` from request body.
2. Redis stores the request count for that key.
3. If count exceeds the limit, request is blocked with **429**.
4. Otherwise, request continues to the actual route handler.
